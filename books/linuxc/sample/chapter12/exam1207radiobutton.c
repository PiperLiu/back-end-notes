...
int main(int argc,char *argv[ ])
{
  ...
  box = gtk_vbox_new(FALSE,0);  //创建一个新的组合盒
  button = gtk_radio_button_new_with_label(group,"Radio Button1");  //创建单选框
  ...
  button = gtk_radio_button_new_with_label(group,"Radio Button2");  //创建单选框
  group = gtk_radio_button_group(GTK_RADIO_BUTTON(button));   //将单选框添加到组中
  gtk_box_pack_start(GTK_BOX(box),button,FALSE,FALSE,0);      //将按钮添加到组合盒中
  gtk_widget_show(button);  //显示按钮

  button = gtk_radio_button_new_with_label(group,"Radio Button3");  //创建单选框
  group = gtk_radio_button_group(GTK_RADIO_BUTTON(button));   //将单选框添加到组中
  gtk_box_pack_start(GTK_BOX(box),button,FALSE,FALSE,0);      //将按钮添加到组合盒中
  gtk_widget_show(button);  //显示按钮
